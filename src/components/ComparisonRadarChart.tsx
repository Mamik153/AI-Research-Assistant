...
import type { ComparisonData } from '../types/research';
...
function BarChartTooltip({ active, payload, label }: BarTooltipProps) {
  ...
  return (
    <div
      className="rounded-lg border border-gray-600 bg-gray-800 px-3 py-2.5 shadow-xl"
      ...
      <div className="space-y-1">
        {payload.map((entry) => (
          <div key={entry.name} className="flex items-center justify-between gap-4 text-sm">
            ...
            <span className="text-gray-300 tabular-nums">{entry.value}</span>
          </div>
        ))}
      </div>
    </div>
...
export const ComparisonRadarChart = ({ comparisonData }: ComparisonRadarChartProps) => {
  ...
  return (
    <div className="rounded-2xl border border-gray-800 bg-gray-900/50 backdrop-blur-sm p-6">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
        ...
        <div className="flex rounded-lg bg-gray-800/80 p-1 gap-0.5">
        ...
        <div className="h-[360px] w-full min-h-0 overflow-x-auto">
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <XAxis type="number" domain={[0, 3]} tick={{ fill: '#6b7280', fontSize: 10 }} />
              <YAxis
                type="category"
                dataKey="subject"
                width={120}
          ...
          <table className="w-full text-sm text-left">
            <thead>
              <tr className="border-b border-gray-700 bg-gray-800/80">
                <th className="px-4 py-3 text-gray-300 font-semibold">Criteria</th>
                {items.map((item, idx) => (
                  <th key={item.name} className="px-4 py-3 font-semibold" style={{ color: COLORS[idx % COLORS.length] }}>
                    {item.name}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {criteria.map((subject, i) => (
                <tr key={i} className="border-b border-gray-700/80 hover:bg-gray-800/40">
                  <td className="px-4 py-3 text-gray-300">{subject}</td>
                  {items.map((item) => (
                    <td key={item.name} className="px-4 py-3 text-gray-400">
                      {item.values[i] ?? '—'}
                    </td>
                  ))}
                </tr>
