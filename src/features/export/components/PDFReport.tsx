...
export const PDFReport = ({ result, diagramDataUris = [] }: PDFReportProps) => {
  ...
  const validDiagramUris = diagramDataUris.filter(
  ...
  return (
    <Document>
      ...
      {/* Overview Section */}
      {structured?.overview &&
        (structured.overview.title || structured.overview.content) && (
          <Page size="A4" style={styles.page}>
      ...
      {/* Benefits & Risks */}
      {structured &&
        ((structured.benefits?.length ?? 0) > 0 ||
              ...
              <>
                ...
                {structured.benefits.map((item: BenefitItem, idx: number) => (
                  <View key={idx} style={styles.benefitRiskCard}>
                    ...
                    <Text style={styles.benefitRiskDesc}>
                      {item.description}
                    </Text>
                  </View>
                ))}
            ...
            {structured.risks && structured.risks.length > 0 && (
              <>
                ...
                {structured.risks.map((item: RiskItem, idx: number) => (
                  <View
                    key={idx}
                    style={[styles.benefitRiskCard, styles.riskCard]}
                  >
                    {item.severity && (
                      <Text style={styles.importanceBadge}>
              ...
              {item.use_cases && item.use_cases.length > 0 && (
                <View>
                  {item.use_cases.map((uc, i) => (
                    <Text key={i} style={styles.useCaseItem}>
                      • {uc}
                    </Text>
                  ))}
                </View>
      ...
      {/* Future Directions */}
      {structured?.future_directions &&
        structured.future_directions.length > 0 && (
          <Page size="A4" style={styles.page}>
            <Text style={styles.sectionTitle}>Future Directions</Text>
            {structured.future_directions.map(
              (item: FutureDirection, idx: number) => (
                <View key={idx} style={styles.benefitRiskCard}>
                  <Text style={styles.benefitRiskTitle}>{item.title}</Text>
                  {item.timeframe && (
                    <Text style={styles.importanceBadge}>
      ...
      {structured?.comparisons &&
        structured.comparisons.criteria?.length > 0 &&
        structured.comparisons.items?.length > 0 && (
          <Page size="A4" style={styles.page}>
            <Text style={styles.sectionTitle}>Comparison</Text>
            <View style={styles.comparisonTable}>
              <View style={[styles.tableRow, styles.tableHeader]}>
                <Text style={[styles.tableCell, { flex: 2 }]}>Item</Text>
                {(structured.comparisons as ComparisonData).criteria.map(
                  (c, i) => (
              ...
              {(structured.comparisons as ComparisonData).items.map(
                (row, idx) => (
          ...
          {structured.timeline.map((item: TimelineEvent, idx: number) => (
            <View key={idx} style={styles.timelineItem}>
              ...
              <View style={styles.timelineContent}>
                ...
                {item.significance && (
                  <Text style={styles.timelineSignificance}>
                    {item.significance}
                  </Text>
                )}
              </View>
      ...
      {result.papers && result.papers.length > 0 && (
        <Page size="A4" style={styles.page}>
          ...
          {result.papers.map((paper: ResearchPaper, idx: number) => (
      ...
      {!result.summary &&
        ...
        (!result.papers || result.papers.length === 0) &&
        !structured?.overview &&
        (structured?.key_concepts?.length ?? 0) === 0 &&
        (structured?.benefits?.length ?? 0) === 0 &&
        (structured?.risks?.length ?? 0) === 0 &&
        (structured?.applications?.length ?? 0) === 0 &&
        (structured?.methodologies?.length ?? 0) === 0 &&
        (structured?.future_directions?.length ?? 0) === 0 &&
        !structured?.comparisons &&
        (structured?.timeline?.length ?? 0) === 0 &&
        (structured?.statistics?.length ?? 0) === 0 &&
        validDiagramUris.length === 0 && (
          <Page size="A4" style={styles.page}>
            <Text style={styles.sectionTitle}>Report Details</Text>
            <Text style={styles.bodyText}>
              No additional content available for this research.
            </Text>
            <View style={styles.footer}>
              <Text>SlickResearch</Text>
              <Text>Page 2</Text>
